...
// Comment related code starts here
...
function updateComment(url, request) {
  const id = Number(url.split("/").filter(element => element)[1]);
  const requestComment = request.body && request.body.comment;
  ...
  if(!requestComment) {
    response.status = 400;
  } else if(!databaseComment || !requestComment.body) {
    response.status = 404;
  } else {
    databaseComment.body = requestComment.body;
    response.status = 200;
    response.body = {comment: databaseComment};
  }
  return response;
};
...
function deleteComment(url, request) {
  ...
  if(!database.comments[id]) {
    ...
    database.articles[article].commentIds.splice(database.articles[article].commentIds.indexOf(id), 1);
    response.status = 204;
  }
  return response;
};
...
function upvoteComment(url, request) {
  ...
  const username = request.body && request.body.username;
  const userInDatabase = database.users[username];

  if(!comment || !username || !userInDatabase){
    response.status = 400;
  } else {
    database.comments[id] = upvote(comment, username);
    response.status = 200;
...
function downvoteComment(url, request) {
  ...
  if(!comment || !username || !userInDatabase){
    ...
    database.comments[id] = downvote(comment, username);
    response.status = 200;
    response.body = {comment: comment};
  }
  return response;
...
// database save and load functions
...
function loadDatabase() {
  // database.yaml has to exist before this function is called
  // it is provided in the main proyect directory
